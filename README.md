...
The current implementation involves the use of a *renderer*, and a *render queue*. Items (called *components*) are drawn on the screen by adding them to the render queue, which is then processed by the renderer. As of now, the render queue has been implemented in a way that allows for a maxmimum of **1024** components. I explicitly chose to not use dynamic memory allocation in order to keep the overhead relatively low. Maybe in the future I'll implement a more sophisticated system that will remove this limitation.

Here's a rundown of the features supported by this interface, and the functions that I'd like to add in the future:

- [X] Drawing images (RGB565 and RGB8565 with binary alpha channel support) from C buffers (generated by the Python scripts);
- [X] Renderer with statically allocated render queue, to provide the user with a simpler interface (`LCD_RendererAdd(...)`, `LCD_RendererRemove()`... functions);
- [ ] Support for custom fonts loaded as C buffers (generated by the Python scripts);
- [ ] Support for alignment for components (e.g. centering text, images), i.e. without having to calculate the position manually;
- [ ] Ability to create custom shapes (associated with a unique ID) made up of more than one basic component (`LCD_BeginComponent`, `LCD_EndComponent`). This would allow for more complex components to be draw on/deleted from the screen without having to manage every single ID manually.
