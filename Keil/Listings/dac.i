# 1 "Source/Libs/DAC/dac.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 397 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Source/Libs/DAC/dac.c" 2
...
# 4 "Source/Libs/DAC/dac.c" 2




static TIMER DAC_SamplesTimer;
static TIMER DAC_SecondsTimer;

static u8 DAC_SinVolume = 5;
static u32 DAC_SinTableIndex = 0;
...
static inline u32 get_tone_timing(u16 note, u8 octave)
{
    ...
    return 25000000 / (freq * 45);
}

static inline u16 adjust_sample_volume(u16 sample, u8 volume)
{
    if (volume == 0)
        return 0;
...
static void next_sample(void)
{
    ...
    ((LPC_DAC_TypeDef *) ((0x40080000UL) + 0x0C000) )->DACR = (volumed_sample << 6);

    if (DAC_SinTableIndex == 45)
        DAC_SinTableIndex = 0;
}
...
void DAC_BUZInit(u8 timer_a, u8 timer_b, u8 int_priority)
{
    ...
    TIMER_Init(&DAC_SamplesTimer, timer_a, 0, int_priority);
    TIMER_Init(&DAC_SecondsTimer, timer_b, 0, int_priority);
}
...
void DAC_BUZSetVolume(u8 volume)
{
    DAC_SinVolume = (((volume) >= (0) && (volume) <= (10))) ? volume : 5;
}
...
void DAC_BUZPlay(DAC_Tone tone, u16 bpm)
{
    ...
    const u32 tone_duration = 25000000 * ((60 * tone.type) / bpm);
...
void DAC_BUZStop(void)
{
    DAC_SinVolume = 5;
    DAC_SinTableIndex = 0;
    ((LPC_DAC_TypeDef *) ((0x40080000UL) + 0x0C000) )->DACR &= ~(0x3FF << 6);
