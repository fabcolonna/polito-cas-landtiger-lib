...
#ifndef __FONT_MSGOTHIC_H
#define __FONT_MSGOTHIC_H
...
/// @brief Each row represents a character with height 16, and contains 8 bits for each row.
///        1 bit represents a pixel, so each character is 8x16 pixels. If i want a bigger
///        font, I need more rows, and for each row, I need more bits to represent each pixel.
///        Since I'm limited to 32 bits for LPC1768, I can't have a font bigger than 32x?.
/// @note The characters are in ASCII order, starting from ' ' (space) and ending in '~' (tilde).
///       This, so that I can directly map the ASCII value of a char (C type) to the index in
///       these arrays, by simply subtracting 32.
