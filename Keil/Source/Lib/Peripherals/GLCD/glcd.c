...
_PRIVATE MEM_Allocator *allocator;
...
_PRIVATE LCD_RQItem render_queue[GLCD_MAX_RENDERABLE_ITEMS] = {0};
_PRIVATE u32 render_queue_free_list[GLCD_MAX_RENDERABLE_ITEMS] = {0};
...
_PRIVATE LCD_Font font_list[GLCD_MAX_FONTS] = {0};
...
{
    if (__LCD_Orientation == LCD_ORIENT_HOR || __LCD_Orientation == LCD_ORIENT_HOR_INV)
    {
...
_PRIVATE bool process_rect(const LCD_Rect *const rect, LCD_Coordinate pos, LCD_BBox *out_bbox, u8 mode)
{
    ...
    if (from.x >= __LCD_MaxX || from.y >= __LCD_MaxY)
                                   ...
                                   LCD_Color bg_col, u16 *out_char_w, u16 *out_char_h)
{
    ...
    *out_char_w = font->char_widths ? font->char_widths[index] : font->max_char_width;
    ...
    if (where.y - baseline_offset < 0 || where.y - baseline_offset >= __LCD_MaxY)
    ...
    if (where.x < 0 || where.x + *out_char_w >= __LCD_MaxX)
...
_PRIVATE bool process_text(const LCD_Text *const text, LCD_Coordinate pos, LCD_BBox *out_bbox, u8 mode)
{
    ...
    while ((chr = *str) && !no_more_space)
    {
        ...
        if ((pos.x + x_inc) < __LCD_MaxX)
            cur_width = (x_diff < __LCD_MaxX) ? x_diff : __LCD_MaxX;
        else
            cur_width = (x_diff + char_w) < __LCD_MaxX ? (x_diff + char_w) : __LCD_MaxX;
...
_PRIVATE LCD_Error unrender_item(LCD_RQItem *const item, bool redraw_underneath)
{
    ...
    for (u32 i = 0, count = 0; i < GLCD_MAX_RENDERABLE_ITEMS && count != render_queue_size; i++)
    {
...
LCD_Error LCD_Init(LCD_Orientation orientation, MEM_Allocator *const alloc, const LCD_Color *const clear_to)
{
    ...
    render_queue_free_list_count = GLCD_MAX_RENDERABLE_ITEMS;
    render_queue_size = 0;
    ...
    for (u32 i = 0; i < GLCD_MAX_RENDERABLE_ITEMS; i++)
        render_queue_free_list[i] = GLCD_MAX_RENDERABLE_ITEMS - 1 - i;

#ifndef __GLCD_USE_MEMORY_ARENA
    allocator = alloc;
#endif
...
{
    return (LCD_Coordinate){__LCD_MaxX, __LCD_MaxY};
}

LCD_Coordinate LCD_GetCenter(void)
{
    return (LCD_Coordinate){__LCD_MaxX / 2, __LCD_MaxY / 2};
}
...
LCD_Color LCD_GetPointColor(LCD_Coordinate point)
{
    ...
    if (point.x >= __LCD_MaxX || point.y >= __LCD_MaxY)
        return dummy;
...
void LCD_SetBackgroundColor(LCD_Color color, bool redraw_objects)
{
    ...
    if (__LCD_ModelCode == HX8347D || __LCD_ModelCode == HX8347A)
    {
        write_to(0x02, 0x00);
        write_to(0x03, 0x00);
        write_to(0x04, 0x00);
    ...
    init_rw_operation_at(0x0022); // Write GRAM
    for (u32 index = 0; index < __LCD_MaxX * __LCD_MaxY; index++)
...
LCD_Error LCD_GetObjBBoxWithID(LCD_ObjID id, LCD_BBox *out_bbox)
{
    ...
    if (id < 0 || id >= GLCD_MAX_RENDERABLE_ITEMS)
        return LCD_ERR_INVALID_OBJ;
...
LCD_Error LCD_GetObjBBox(const LCD_Obj *const obj, LCD_BBox *out_bbox)
{
    ...
    // assert(obj->comps_size < GLCD_MAX_COMPS_PER_OBJECT); // Should never have all these components.
...
LCD_Error LCD_RQAddObject(const LCD_Obj *const obj, LCD_ObjID *out_id, u8 options)
{
    ...
    // Allocating the object using the allocator.
    obj_ref = MEM_Alloc(allocator, sizeof(LCD_Obj));
    if (!obj_ref)
        return LCD_ERR_NOT_ENOUGH_SPACE;

    // Allocating the object components
    LCD_Component *comps = MEM_Alloc(allocator, obj->comps_size * sizeof(LCD_Component));
    if (!comps)
    {
        MEM_Free(allocator, obj_ref);
        return LCD_ERR_NOT_ENOUGH_SPACE;
    }
...
LCD_Error LCD_RQRender(void)
{
    ...
    // GLCD_MAX_RENDERABLE_ITEMS, but that would be surely more inefficient.

    bool res = true;
    LCD_RQItem *item;
    for (u32 i = 0, count = 0; i < GLCD_MAX_RENDERABLE_ITEMS && count != render_queue_size && res; i++)
    {
        item = &render_queue[i];
        if (item->id == -1 && !item->obj)
            continue;
...
LCD_Error LCD_RQRemoveObject(LCD_ObjID id, bool redraw_underneath)
{
    ...
    MEM_Free(allocator, item->obj->comps);
    MEM_Free(allocator, item->obj);
...
#ifdef __GLCD_USE_MEMORY_ARENA
    LCD_Error err;
#endif

    u32 count = 0;
    for (u32 i = 0; i < GLCD_MAX_RENDERABLE_ITEMS && count != render_queue_size; i++)
    {
        item = &render_queue[i];
        if (item->id == -1 || !item->obj)
            continue;
...
{
    if (font_list_size >= GLCD_MAX_FONTS)
